...
| Required API Keys | OpenHome API Keys は使わず、`config.py` の placeholder を置き換える |

## Authentication

OpenHome does not currently provide YouTube OAuth through Linked Accounts for this community Ability. Because of that, this version uses a manual setup:
...
- Public channel lookup can optionally use a manual YouTube Data API key: `YOUTUBE_API_KEY`.
- Real API keys, OAuth client secrets, refresh tokens, `credentials.json`, `token.json`, `.env`, and `*.secret` files must never be committed to GitHub.
...
This manual credential setup is a temporary workaround for the community version until YouTube OAuth is available through OpenHome Linked Accounts.
...
## 非公開ライブで必要な config.py 設定

非公開ライブや自分の配信を確実に追うには OAuth が必要です。この Ability では OpenHome Third Party API Keys を使わず、Ability 内の `config.py` から認証情報を読みます。
...
公開ライブだけを API key で試す場合は、任意で次も設定できます。
...
OS 環境変数 `YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET`, `YOUTUBE_REFRESH_TOKEN`, `YOUTUBE_API_KEY` が設定されている場合は、`config.py` より優先されます。`.env` と `youtube_live_companion.env` は runtime で読みません。
...
# Optional. Public channel lookup only.
...
| `YOUTUBE_API_KEY` | YouTube Data API key。公開ライブ検索用なので任意 |

### 4. 非秘密設定も config.py に入れる

対象ライブや polling の調整も `config.py` で行います。必要なら次の値を変更します。
...
非公開ライブの検証では `VIDEO_ID` や `CHANNEL_ID` を空のままにしても、OAuth で `mine=true` の配信を自動取得します。公開ライブを API key だけで探す場合は、`YOUTUBE_API_KEY` と `CHANNEL_ID` を組み合わせます。

`config.py` を保存したら、Agent を restart してから `配信設定` または `配信ステータス` をもう一度試します。
...
`POLL_INTERVAL_SECONDS` などの調整値や、公開ライブ用の `CHANNEL_ID` / `VIDEO_ID` / `LIVE_CHAT_ID` は、OpenHome の Live Editor で `config.py` を編集します。
...
4. 公開ライブ検索も使う場合は `YOUTUBE_API_KEY` も設定する。
5. `ASSISTANT_LANGUAGE` を `ja` または `en` に設定する。
6. `CHANNEL_ID`, `VIDEO_ID`, `LIVE_CHAT_ID` や polling 設定を調整する。
7. 保存する。
8. Agent を restart する。
...
1. `config.py` の `YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET`, `YOUTUBE_REFRESH_TOKEN`, `YOUTUBE_API_KEY`
...
公開ライブだけを API key で試す場合は、`config.py` に `YOUTUBE_API_KEY` と `CHANNEL_ID` を入れます。
