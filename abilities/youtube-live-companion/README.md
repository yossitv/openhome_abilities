...
| Image | `abilities/youtube-live-companion/icon.png` |
...
Ability の `Image` / `image_file` には、SVG ではなく画像ファイルを指定します。この Ability のアイコン PNG は次に置いています。

```text
abilities/youtube-live-companion/icon.png
```

`scripts/package-abilities.sh youtube-live-companion` で zip を作ると、zip 内には `youtube-live-companion/icon.png` として含まれます。Dashboard の `Image` に別途ファイル指定が必要な場合は、ローカルの `abilities/youtube-live-companion/icon.png` を選んでください。
