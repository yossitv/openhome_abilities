...
| Required API Keys | `config.py` の OAuth placeholder を置き換える |

## Authentication

OpenHome does not currently provide YouTube OAuth through Linked Accounts for this Ability. Because of that, this version uses OAuth credentials configured in `config.py`.

- Private or unlisted lives require YouTube OAuth credentials: `YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET`, and `YOUTUBE_REFRESH_TOKEN`.
- Replace the placeholders in this Ability's `config.py` from the OpenHome Ability Editor.
- Real OAuth client secrets and refresh tokens must never be committed to GitHub. Keep committed `config.py` values as placeholders only.
...
この Ability は OAuth で自分の YouTube Live を探します。認証情報は、この Ability ルートにある `config.py` から読みます。
...
OpenHome Ability Editor では、`config.py` は `main.py` や `background.py` と同じ階層にあるファイルとして編集します。
...
Ability zip に含まれる `config.py` には placeholder だけを置いています。自分の OpenHome Ability Editor 上で `config.py` を開き、ローカルの実値に差し替えます。
...
コメント要約、無音時の声かけ、ステータス確認、設定確認、リセット結果などの出力言語は、同じ `config.py` の `ASSISTANT_LANGUAGE` で設定します。
...
話し方を調整したい場合は、`SUMMARY_SYSTEM_PROMPTS`、`QUIET_SYSTEM_PROMPTS`、`MAIN_SPEECH_MESSAGES` の `ja` / `en` の文面を編集します。
...
認証情報は、`config.py` の `YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET`, `YOUTUBE_REFRESH_TOKEN` に設定します。実値は GitHub に入れず、自分の OpenHome Ability Editor 上でだけ置き換えてください。
...
OpenHome Ability Editor でも、`config.py`, `main.py`, `background.py`, `icon.png` が同じ階層にある前提です。

Ability の `Image` / `image_file` には、SVG ではなく画像ファイルを指定します。この Ability のアイコン PNG は Ability ルートに置いています。

```text
icon.png
```

Dashboard の `Image` に別途ファイル指定が必要な場合は、この Ability の `icon.png` を選んでください。
